...
评估当前 BoW 模型在数据库样本上的性能指标（支持随机采样）：
- 准确率 (Accuracy)
- 精确率 (Precision)
- 召回率 (Recall)
- F1 分数 (F1 Score)
...
使用方法:
    python tools/evaluate_bow_model.py <profile_name> [--sample-size N]
...
示例:
    python tools/evaluate_bow_model.py default
    python tools/evaluate_bow_model.py 4claude --sample-size 100
    python tools/evaluate_bow_model.py default --sample-size 0  # 全量评估
"""
...
from ai_proxy.moderation.smart.bow import bow_model_exists, bow_predict_proba


def evaluate_bow_model(profile_name: str, sample_size: int = 100):
    """
    评估 BoW 模型性能
    ...
    # 检查模型是否存在
    if not bow_model_exists(profile):
        print(f"❌ BoW 模型不存在")
    ...
    for i, sample in enumerate(samples):
        ...
        try:
            # 预测违规概率
            violation_prob = bow_predict_proba(sample.text, profile)
    ...
    if accuracy < 0.8:
        ...
        print(f"     - 调整模型参数 (max_features, ngram_range 等)")
...
示例:
  ...
  python tools/evaluate_bow_model.py default --sample-size 0  # 全量评估
    ...
    try:
        evaluate_bow_model(args.profile_name, args.sample_size)
